...
public class ClimbingDetector : MonoBehaviour
{
    ...
    {
        LayerMask collisionLayerMask = 1 << collision.gameObject.layer;//Bitwise operation
        if ((collisionLayerMask & climbingLayerMask) != 0)// logical AND operation, if they are different the result is not 0
        {
            CanClimb = true;
        }
    }

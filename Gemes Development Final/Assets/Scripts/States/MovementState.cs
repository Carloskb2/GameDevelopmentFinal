...
public class MovementState : State
{
    ...
    {
        if (TestFallTransition())//return if character is falling to avoid transition to idle
            return;
        CalculateVelocity();
        SetPlayerVelocity();
    ...
    protected void CalculateSpeed(Vector2 movementVector, MovementData movementData)
    {
        ...
        movementData.currentSpeed = Mathf.Clamp(movementData.currentSpeed, 0, agent.agentData.maxSpeed);//clamp movementData.currentSpeed between 0 and max speed
    }
}
